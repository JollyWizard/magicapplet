...
//	_axes.setVisible(true);
	...
	_lowerGraph = _axes.getPolyLineFromFunction(NUM_INTERVALS,0,13,_curveFunction);
	_curveFunction.setRadius(radius);
	_upperGraph = _axes.getPolyLineFromFunction(NUM_INTERVALS,0,13,_curveFunction);
	...
	double leftXLocal = 4;
	DPoint leftPointLocal = new DPoint(leftXLocal,_curveFunction.getYofX(leftXLocal));
	DPoint leftPointPanel = _axes.transformLocalToPanel(new DPoint(leftPointLocal));
	double rightXLocal = 12;
	DPoint rightPointLocal = new DPoint(rightXLocal,_curveFunction.getYofX(rightXLocal));
	DPoint rightPointPanel = _axes.transformLocalToPanel(new DPoint(rightPointLocal));
	...
	_tangentLine.setLine(_secantTriangle.getTangentPoint(),_secantTriangle.getSecantPoint());
...
//	_curveEquationLabel.setPosition(422,230);
	...
	_componentList.add(0,_secantTriangle);//needs to be on top because of setSyncParams()
	...
	if (_xValuePanel<282) _xValuePanel=282;
	if (_xValuePanel>550) _xValuePanel=550;
	DPoint curvePt = new DPoint(_xValuePanel,_axes.getOrigin().y);//on the x axis in Panel coords
	_axes.transformPanelToLocal(curvePt);//on the x axis in Local coords
	curvePt.y = _curveFunction.getYofX(curvePt.x);//on curve in Local coords
	...
	_tangentLine.setLine(_secantTriangle.getTangentPoint(),new DPoint(_xValuePanel,_yValuePanel));
	_tangentLine.setLength(0,1500);
    }
    private void syncDeltaYXLabels() {
	DPoint secantPoint = new DPoint(_xValuePanel,_yValuePanel);
	double xPos = (_secantTriangle.getTangentPoint().x+secantPoint.x)/2-_deltaXLabel.getRect().width/2;
	double yPos = _secantTriangle.getTangentPoint().y;
	_deltaXLabel.setPosition(xPos,yPos);
	_dxLabel.setPosition(xPos,yPos);

	xPos = secantPoint.x;
	yPos = (_secantTriangle.getTangentPoint().y+secantPoint.y)/2-_deltaXLabel.getRect().height/2;
...
//		_tangentLine.setVisible(true);
	...
	if (_componentList.get(0)==_secantTriangle) {setSyncParamsFromSecantTriangle();}
    }
    protected void syncComponents() {
	for (int i=0; i<_componentList.size(); i++) {
    	    if (_componentList.get(i)==_secantTriangle) {syncSecantTriangle();}
    	    else if (_componentList.get(i)==_tangentLine) {syncTangentLine();}
    	    else if (_componentList.get(i)==_deltaXLabel) {syncDeltaYXLabels();}
	}
