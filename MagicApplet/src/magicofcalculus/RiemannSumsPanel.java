...
public class RiemannSumsPanel extends Panel {
	...
	_graph = _axes.getPolyLineFromFunction(NUM_INTERVALS,0,lengthOfAxisXLocal,_function);
	...
	_rightPoint.setCenter(_leftXPanel+(_rightXPanel-_leftXPanel)/4,origin.y);	
...
//	_axes.setRiemannRectsStrokeWidth(4);
	...
	_dxDimensionLabel.setDraggable(true);//dev, for locating permanent position
	...
	_dxLine.setLine(_axes.transformLocalToPanel(new DPoint(lineXValueLocal,0)),_axes.transformLocalToPanel(new DPoint(lineXValueLocal,lineYValueLocal)));
...
//	setScene(0);
	...
	if (xValuePanelWasEqualToLeftXPanel) {
	    if (_xValuePanel==_leftXPanel+1) _xValuePanel=_leftXPanel+2;
	    else if (_xValuePanel<_leftXPanel) _xValuePanel=_leftXPanel;
	}
	else {
	    if (_xValuePanel<=_leftXPanel+1) _xValuePanel=_leftXPanel;
	}
	...
	for (int i=0; i<_componentList.size(); i++) {
		...
		_axes.setRiemannRects(_leftXLocal,_rightXLocal,deltaXLocal,_function);
