...
//	setBackButtonArea(0,lowerOrigin.y,lowerOrigin.x,Panel.PANEL_HIEGHT-lowerOrigin.y);
	...
	_lowerGraph = _lowerAxes.getPolyLineFromFunction(NUM_INTERVALS,0,lengthOfAxisXLocal,_curveFunction);
//	_lowerGraph.setColor(Color.red);

	_upperGraph = _upperAxes.getPolyLineFromFunction(NUM_INTERVALS,0,lengthOfAxisXLocal,_areaFunction);
	...
	double oldXValuePanel = _xValuePanel;//save the old _xValuePanel for repainting
	//set _xValuePanel
	if (_componentList.get(0)==_lowerPoint) {_xValuePanel = _lowerPoint.getCenter().x;}
	else if (_componentList.get(0)==_upperPoint) {_xValuePanel = _upperPoint.getCenter().x;}
	//make sure _xValuePanel is within limits
	if (_xValuePanel<_lowerAxes.getOrigin().x) _xValuePanel = _lowerAxes.getOrigin().x;
	else if (_xValuePanel>_lowerAxes.getOrigin().x+_lowerAxes.getXAxisLengthInPanel()) _xValuePanel = _lowerAxes.getOrigin().x+_lowerAxes.getXAxisLengthInPanel();
	//repaint dragged over region
	if (oldXValuePanel<_xValuePanel) repaint((int)oldXValuePanel-1,0,(int)(_xValuePanel-oldXValuePanel)+2,PANEL_HIEGHT);
	else repaint((int)_xValuePanel-1,0,(int)(oldXValuePanel-_xValuePanel)+2,PANEL_HIEGHT);
    }
	...
	//_upperPoint
	DPoint point = new DPoint(_xValuePanel,_upperAxes.getOrigin().y);//on the upper x axis in Panel coords
	...
	_lowerAxes.setFillUnderCurve(_lowerGraph,_lowerAxes.getOrigin().x,_xValuePanel);
	_lowerAxes.setFillUnderCurveVisible(_xValuePanel != _lowerAxes.getOrigin().x);
	syncThreeLabel();
    }
