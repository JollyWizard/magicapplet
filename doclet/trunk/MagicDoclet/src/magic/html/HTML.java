...
public abstract class HTML extends XML implements HtmlContents,
  ...
  public String toHTML () {
    ...
    if (!closed) r.append(close());
    r
            .append(new Comment(name, " ", classes.toDeclaration())
                    .toHTML());
    return r.toString();
  }
   ...
   * the HTML.Iterators to ensure that modifications are propogated to multiple
   * iterators.
   * 
   * @author James Arlow
   */
  public static class HtmlIterator implements Iterator<HtmlContents> {
    ...
    @Override
    public void remove () {
      if (removed || index == -1) throw new IllegalStateException();
  ...
  public class Iterators {
    ...
    private List<SoftReference<HtmlIterator>> deadIterators =
            new LinkedList<SoftReference<HtmlIterator>>();

    private List<SoftReference<HtmlIterator>> openIterators =
            new LinkedList<SoftReference<HtmlIterator>>();
    ...
    protected HtmlIterator newIterator (HTML parent) {
      ...
      SoftReference<HtmlIterator> ref =
              new SoftReference<HtmlIterator>(r);
