...
public class Tr extends HTML {
  ...
  public static Tr columnHeaders (Class<? extends Tr> cl) {
    if (cl == null || cl.equals(Tr.class)) return null;
    Tr r = new Tr();
    HashMap<Integer, List<Field>> fieldMap =
            Map.er.process(Td.index.class, cl);
    for (List<Field> lf : fieldMap.values()) {
      for (Field f : lf) {
        ...
        if (colspan != null) header.colspan.set(colspan.value());
        header.setScopeCol();
  ...
  public String toHTML () {
    ...
    HashMap<Integer, List<Td>> fieldMap =
            Map.er.apply(Td.index.class, this, Td.class);
