...
public class Axes extends Component
{
  ...
  /**
   * Class constructor
   * @param panel The Panel representing the Axes
   */
 ...
 */
  ...
  public PolyLine getPolyLineFromFunction(int numIntervals, double leftXLocal,
      double rightXLocal, Function curveFunction) {
   ...
   * @param set Boolean indicating if the fill should be visible or not
   */
   ...
   * @param color Color to assign to the fill
   */
 ...
 * @param fromXInPanel Starting X coordinate
 * @param toXInPanel Ending X coordinate
 */
  public void setFillUnderCurve(PolyLine curve, double fromXInPanel,
      double toXInPanel) {
    repaint();
    _underTheCurvePath.reset();
    ArrayList<DPoint> pointList = curve.getPointList(fromXInPanel, toXInPanel);
    ...
    _underTheCurvePath.lineTo((int) round(toXInPanel), (int) round(_origin.y));
    _underTheCurvePath
        .lineTo((int) round(fromXInPanel), (int) round(_origin.y));
    _underTheCurvePath.closePath();
    repaint();
  }

  /**
   * Renders a fill between the specified curves between the specified starting
  ...
  public void setFillBetweenCurves(PolyLine curve1, PolyLine curve2,
    ...
    repaint();
    _underTheCurvePath.reset();
    ArrayList<DPoint> pointList1 =
        curve1.getPointList(fromXInPanel, toXInPanel);
    ArrayList<DPoint> pointList2 =
        curve2.getPointList(fromXInPanel, toXInPanel);
  ...
  public void setRiemannRects(double fromXLocal, double toXLocal,
    ...
    for (int i = 0; i < numPoints - 1; i++) {
      ...
      double heightOfRect = curveFunction.getYofX(xList[i + isRightEndPoints]);
      // double heightOfRect =
