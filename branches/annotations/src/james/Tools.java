...
public class Tools {
      ...
      if (f.getType().equals(Label.class)) {
        try {
          if (f.get(p) == null) {
            ...
            if (l == null) continue fields;

            if (lp != null)
              l.setPosition(new DPoint(lp.pos().x(), lp.pos().y()));
            p._componentList.add(l);
            System.out.println("LABEL BUILT: "
                    + p.getClass().getName()
                    + f.getAnnotation(LabelProperties.class).image());
          } else
  ...
  /**
   * Builds all Axes and PolyLines possible from config parameters and adds them
  ...
  public static void buildAxes (Panel p) {
    ...
    do {
      ...
      if (c != Panel.class) c = c.getSuperclass();
    } while (c != Panel.class);
  ...
  public static void buildQuadCurve (Panel p, Field f) {
    ...
    QuadCurveProperties qp =
            f.getAnnotation(QuadCurveProperties.class);
    if (qp == null) return;
  ...
  public static void buildAxis (Panel p, Field f) {
    ...
    try {
      Axes a = new Axes(p);
      a.setAxesInPanel(new DPoint(ap.origin().x(), ap.origin().y()),
              ap.width(), ap.height());
  ...
  /**
   * Stores axis indexes so that multi-graph panels can link to correct graphs.
   */
  public static HashMap<Panel, HashMap<Integer, Axes>> axesIndexes =
          new HashMap<Panel, HashMap<Integer, Axes>>();
  ...
  public static Color getColor (String s) {
    ...
    try {
      ...
      if (f != null && Modifier.isStatic(f.getModifiers())
              && f.getType().equals(Color.class)) { return (Color) f
              .get(null); }
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  public static PolyLine getPolyLineFromFunction (Axes axes,
    ...
    try {
      ...
      PolyLine r =
              axes.getPolyLineFromFunction(config.intervals(), config
                      .leftXLocal(), config.rightXLocal(), function);
